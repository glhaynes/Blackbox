...
uint64_t m6502_tick(m6502_t* c, uint64_t pins) {
    if (pins & (M6502_SYNC|M6502_IRQ|M6502_NMI|M6502_RDY|M6502_RES)) {
        ...
        if (pins & M6502_SYNC) {
            ...
            //int nmival = M6502_NMI;    // 0x8000000
            //int resval = M6502_RES;    // 0x40000000
    ...
    /* BRK  */
        case (0x00<<3)|0:
            ...
            break;
        case (0x00<<3)|1:
            if(0==(c->brk_flags&(M6502_BRK_IRQ|M6502_BRK_NMI))){
                c->PC++;
            }
            _SAD(0x0100|c->S--,c->PC>>8);
            if(0==(c->brk_flags&M6502_BRK_RESET)){
                _WR();
        ...
        case (0x00<<3)|2:
            ...
            break;
        case (0x00<<3)|3:
            _SAD(0x0100|c->S--,c->P|M6502_XF);
            if(c->brk_flags&M6502_BRK_RESET){
                c->AD=0xFFFC;
            }else{
                _WR();
                if(c->brk_flags&M6502_BRK_NMI){
                    c->AD=0xFFFA;
                }else{
                    c->AD=0xFFFE;
                }
        ...
        case (0x00<<3)|4:
            _SA(c->AD++);
            c->P|=(M6502_IF|M6502_BF);
            c->brk_flags=0; /* RES/NMI hijacking */
            break;
        case (0x00<<3)|5:
            _SA(c->AD);
            c->AD=_GD();
            /* NMI "half-hijacking" not possible */
            break;
        case (0x00<<3)|6:
            c->PC=(_GD()<<8)|c->AD;
            _FETCH();
